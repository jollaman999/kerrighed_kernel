				   ...
				   struct sem_array *received_sma)
{
	...
	*sma = *received_sma;
	...
	INIT_LIST_HEAD(&sma->remote_pending_const);
				     ...
				     struct sem_array *received_sma)
{
	...
	list_for_each_entry_safe(q, tq, &received_sma->remote_pending_alter, list) {

		int is_local = 0;

		/* checking if the sem_queue is local */
		list_for_each_entry(local_q, &sma->pending_alter, list) {
	...
	next1:
		...
		if (is_local)
			...
			list_add(&q->list, &sma->remote_pending_alter);
	}

	list_for_each_entry_safe(q, tq, &received_sma->remote_pending_const, list) {

		int is_local = 0;

		/* checking if the sem_queue is local */
		list_for_each_entry(local_q, &sma->remote_pending_const, list) {
	...
	next2:
		...
		if (is_local)
			...
			list_add(&q->list, &sma->remote_pending_const);
	}
}
				      ...
				      const struct sem_array* sma)
{
	...
	list_for_each_entry(q, &sma->remote_pending_alter, list)
		nb_sem_pending++;
				    ...
				    semarray_object_t *sem_object)
{
	...
	INIT_LIST_HEAD(&sem_object->imported_sem.pending_alter);
	INIT_LIST_HEAD(&sem_object->imported_sem.pending_const);
	INIT_LIST_HEAD(&sem_object->imported_sem.remote_pending_alter);
	INIT_LIST_HEAD(&sem_object->imported_sem.remote_pending_const);
