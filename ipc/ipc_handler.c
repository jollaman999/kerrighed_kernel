...
{
	ipcmap_object_t *ipc_map, *max_idx;
	int i = 1, id = -1, offset;

	max_idx = _kddm_grab_object(ids->krgops->map_kddm_set, 0);
	...
	while (id == -1) {
		...
		if (ipc_map->alloc_map != ULONG_MAX) {
			...
			if (offset < BITS_PER_LONG) {
				...
				if (id >= max_idx->alloc_map)
					max_idx->alloc_map = id + 1;
			}
		}
...
void krg_ipc_rmid(struct ipc_ids* ids, int index)
{
	...
	/* Check if max_idx must be adjusted */

	max_idx = _kddm_grab_object(ids->krgops->map_kddm_set, 0);

	if (max_idx->alloc_map != index + 1)
		goto done;
