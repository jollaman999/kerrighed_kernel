					   ...
					   int nsems)
{
	...
	for (i = 0; i < nsems; i++) {
		INIT_LIST_HEAD(&sem_object->imported_sem.sem_base[i].pending_alter);
		INIT_LIST_HEAD(&sem_object->imported_sem.sem_base[i].pending_const);
		INIT_LIST_HEAD(&sem_object->imported_sem.sem_base[i].remote_pending_alter);
		INIT_LIST_HEAD(&sem_object->imported_sem.sem_base[i].remote_pending_const);
	}
