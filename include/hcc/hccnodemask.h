...
static inline hcc_node_t __next_krgnode_in_ring(hcc_node_t node,
...
static inline hcc_node_t __nth_krgnode(hcc_node_t node,
					     const krgnodemask_t *v)
{
	hcc_node_t iter;
...
static inline int __krgnode_is_unique(hcc_node_t node,
				      const krgnodemask_t *v)
{
  int i;
...
#define krgnode_online(node)		((node) == hcc_node_id)
#define krgnode_possible(node)	((node) == hcc_node_id)
#define krgnode_present(node)	((node) == hcc_node_id)

#define any_online_krgnode(mask)		hcc_node_id
#endif
