...
static void handle_faf_getdents(struct grpc_desc *desc, void *__msg, size_t size)
{
...
send_error:
	err_grpc = grpc_pack_type(desc, err);
	if (err <= 0 || err_grpc)
		goto out;

	/* err contains the used size of the buffer */
	err_grpc = grpc_pack(desc, 0, dirent, err);
	if (err_grpc)
		grpc_cancel(desc);
...
static
struct faf_polled_fd_node *
faf_polled_fd_find_node(struct faf_polled_fd *polled_fd, hcc_node_t node)
{
	struct faf_polled_fd_node *polled_fd_node;
...
static int faf_polled_fd_add(hcc_node_t client,
...
static int faf_polled_fd_remove(hcc_node_t client,
				unsigned int server_fd,
				unsigned long dvfs_id)
{
	struct dvfs_file_struct *dvfs_file;
...
void faf_server_init (void)
{
	...
	grpc_register_void(GRPC_FAF_GETDENTS, handle_faf_getdents, 0);
	faf_poll_init();
