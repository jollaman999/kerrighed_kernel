					      ...
					      const char *name)
{
	...
	/* add ID to the list */
	/*
	 * if particular gscheduler already handles all the processes, there is
				     ...
				     const char *page, size_t count)
{
	...
	if (sscanf(page, "%hd", &val) != 1) {
		...
		if (val <= 0 && process_set_contains_all(pset)) {
			 ...
			 * gscheduler doesn't handle all processes.
			 */
			pset->handle_all = 0;
			 ...
			 * gscheduler handles all processes.
			 */
			     ...
			     struct pid *pid, enum pid_type type)
{
	 ...
	 * process set links are protected by hcc_gscheduler_subsys.su_mutex
	 */
