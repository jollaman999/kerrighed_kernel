...
/*
 * Helper functions to make configfs operations on hcc's gschedulers global
 *
...
static struct vfsmount *gscheduler_fs_mount; /* vfsmount attached to configfs */
...
static inline int in_hcc_gscheduler_subsys(struct config_item *item)
{
	return item && item != &hcc_gscheduler_subsys.su_group.cg_item;
}
...
static int item_path_length(struct config_item *item)
{
	...
	do {
		...
		p = p->ci_parent;
	} while (in_hcc_gscheduler_subsys(p));
	return length;
}
...
static void fill_item_path(struct config_item *item, char *buffer, int length)
{
	...
	--length;
	for (p = item; in_hcc_gscheduler_subsys(p); p = p->ci_parent) {
 ...
 * the configfs gscheduler subsystem entry.
 *
...
static struct dentry *get_child_dentry(const char *child_name)
{
	...
	d_dir = dget(hcc_gscheduler_subsys.su_group.cg_item.ci_dentry);

	last_child_comp = strrchr(child_name, '/');
	if (last_child_comp) {
		struct nameidata nd;

		err = vfs_path_lookup(d_dir, gscheduler_fs_mount,
...
/**
 * Change current's root to configfs gscheduler subsystem'root, and save
 ...
 */
static void chroot_to_gscheduler_subsystem(struct path *prev_root)
{
	...
	*prev_root = current->fs->root;
	...
	new_root.mnt = gscheduler_fs_mount;
	new_root.dentry = hcc_gscheduler_subsys.su_group.cg_item.ci_dentry;
				       ...
				       void *_msg, size_t size)
{
	...
	chroot_to_gscheduler_subsystem(&old_root);
 ...
 *			configfs gscheduler subsystem directory.
...
static struct config_item *get_item(const char *path)
{
	...
	child = &hcc_gscheduler_subsys.su_group.cg_item;
	 ...
	 * configfs tree, but it is safe with hcc_gscheduler subtree since all
	 * directories are config_groups.
	 */

	parent_root = __path;

	mutex_lock(&hcc_gscheduler_subsys.su_mutex);
	do {
	...
	mutex_unlock(&hcc_gscheduler_subsys.su_mutex);
...
int global_config_start(void)
{
	...
	if (!fs_type)
		return -ENODEV;
	err = simple_pin_fs(fs_type, &gscheduler_fs_mount, &mount_count);
	if (err)
		goto err_pin_fs;
...
err_set:
	...
	simple_release_fs(&gscheduler_fs_mount, &mount_count);
err_pin_fs:
	goto out;
}
