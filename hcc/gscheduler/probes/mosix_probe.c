/*
 *  hcc/gscheduler/probes/mosix_probe.c
 *
...
struct mosix_probe_info {
	struct hcc_gsched_module_info module_info;
...
static inline
struct mosix_probe_info *
to_mosix_probe_info(struct hcc_gsched_module_info *sched_info)
{
...
static struct gscheduler_probe *mosix_probe;
...
static struct gscheduler_probe_source *mosix_probe_sources[NR_VALUES + 1];
...
mosix_probe_info_copy(struct task_struct *task,
		      struct hcc_gsched_module_info *info)
{
...
static void mosix_probe_info_free(struct hcc_gsched_module_info *info)
{
...
static struct hcc_gsched_module_info_type mosix_probe_module_info_type = {
	.name = "mosix probe",
	.owner = THIS_MODULE,
...
static struct mosix_probe_info *get_mosix_probe_info(struct task_struct *task)
{
	...
	mod_info = hcc_gsched_module_info_get(task,
...
DEFINE_GSCHEDULER_PROBE_SOURCE_SHOW(value_mean_load, page)
{
	return sprintf(page, "%lu\n", mosix_data.mosix_mean_load);
}

/* DEFINE_GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_mean_load) */
/* { */
/*	int isChanged = 0; */
...
static BEGIN_GSCHEDULER_PROBE_SOURCE_TYPE(value_mean_load),
	.GSCHEDULER_PROBE_SOURCE_GET(value_mean_load),
	.GSCHEDULER_PROBE_SOURCE_SHOW(value_mean_load),
	.GSCHEDULER_PROBE_SOURCE_VALUE_TYPE(value_mean_load, unsigned long),
/*	.GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_mean_load), */
END_GSCHEDULER_PROBE_SOURCE_TYPE(value_mean_load);

DEFINE_GSCHEDULER_PROBE_SOURCE_GET(value_upper_load, unsigned long, value_p, nr)
{
	*value_p = upper_load;
	return 1;
}

DEFINE_GSCHEDULER_PROBE_SOURCE_SHOW(value_upper_load, page)
{
        return sprintf(page, "%lu\n", mosix_data.mosix_upper_load);
}

/* DEFINE_GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_upper_load) */
/* { */
/*         int isChanged = 0; */
...
static BEGIN_GSCHEDULER_PROBE_SOURCE_TYPE(value_upper_load),
	.GSCHEDULER_PROBE_SOURCE_GET(value_upper_load),
	.GSCHEDULER_PROBE_SOURCE_SHOW(value_upper_load),
	.GSCHEDULER_PROBE_SOURCE_VALUE_TYPE(value_upper_load, unsigned long),
/*	.GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_upper_load), */
END_GSCHEDULER_PROBE_SOURCE_TYPE(value_upper_load);

DEFINE_GSCHEDULER_PROBE_SOURCE_GET(value_single_process_load,
...
DEFINE_GSCHEDULER_PROBE_SOURCE_SHOW(value_single_process_load, page)
{
        return sprintf(page, "%lu\n", mosix_data.mosix_single_process_load);
}

/* DEFINE_GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_single_process_load) */
/* { */
/*         int isChanged = 0; */
...
static BEGIN_GSCHEDULER_PROBE_SOURCE_TYPE(value_single_process_load),
	.GSCHEDULER_PROBE_SOURCE_GET(value_single_process_load),
	.GSCHEDULER_PROBE_SOURCE_SHOW(value_single_process_load),
	.GSCHEDULER_PROBE_SOURCE_VALUE_TYPE(value_single_process_load,
					   unsigned long),
/*	.GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_single_process_load), */
END_GSCHEDULER_PROBE_SOURCE_TYPE(value_single_process_load);

DEFINE_GSCHEDULER_PROBE_SOURCE_GET(value_norm_mean_load,
				  unsigned long, value_p, nr)
{
...
DEFINE_GSCHEDULER_PROBE_SOURCE_SHOW(value_norm_mean_load, page)
{
        return sprintf(page, "%lu\n", mosix_data.mosix_norm_mean_load);
}

/* DEFINE_GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_norm_mean_load) */
/* { */
/*         int isChanged = 0; */
...
static BEGIN_GSCHEDULER_PROBE_SOURCE_TYPE(value_norm_mean_load),
	.GSCHEDULER_PROBE_SOURCE_GET(value_norm_mean_load),
	.GSCHEDULER_PROBE_SOURCE_SHOW(value_norm_mean_load),
	.GSCHEDULER_PROBE_SOURCE_VALUE_TYPE(value_norm_mean_load, unsigned long),
/*	.GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_norm_mean_load), */
END_GSCHEDULER_PROBE_SOURCE_TYPE(value_norm_mean_load);

DEFINE_GSCHEDULER_PROBE_SOURCE_GET(value_norm_upper_load,
				  unsigned long, value_p, nr)
{
...
DEFINE_GSCHEDULER_PROBE_SOURCE_SHOW(value_norm_upper_load, page)
{
        return sprintf(page, "%lu\n", mosix_data.mosix_norm_upper_load);
}

/* DEFINE_GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_norm_upper_load) */
/* { */
/*         int isChanged = 0; */
...
static BEGIN_GSCHEDULER_PROBE_SOURCE_TYPE(value_norm_upper_load),
	.GSCHEDULER_PROBE_SOURCE_GET(value_norm_upper_load),
	.GSCHEDULER_PROBE_SOURCE_SHOW(value_norm_upper_load),
	.GSCHEDULER_PROBE_SOURCE_VALUE_TYPE(value_norm_upper_load,
					   unsigned long),
/*	.GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_norm_upper_load), */
END_GSCHEDULER_PROBE_SOURCE_TYPE(value_norm_upper_load);

DEFINE_GSCHEDULER_PROBE_SOURCE_GET(value_norm_single_process_load,
		       unsigned long, value_p, nr)
{
...
DEFINE_GSCHEDULER_PROBE_SOURCE_SHOW(value_norm_single_process_load, page)
{
        return sprintf(page, "%lu\n",
		       mosix_data.mosix_norm_single_process_load);
}

/* DEFINE_GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_norm_single_process_load) */
/* { */
/*         int isChanged = 0; */
...
static BEGIN_GSCHEDULER_PROBE_SOURCE_TYPE(value_norm_single_process_load),
	.GSCHEDULER_PROBE_SOURCE_GET(value_norm_single_process_load),
	.GSCHEDULER_PROBE_SOURCE_SHOW(value_norm_single_process_load),
	.GSCHEDULER_PROBE_SOURCE_VALUE_TYPE(value_norm_single_process_load,
					   unsigned long),
/*	.GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(value_norm_single_process_load), */
END_GSCHEDULER_PROBE_SOURCE_TYPE(value_norm_single_process_load);

DEFINE_GSCHEDULER_PROBE_SOURCE_GET_WITH_INPUT(value_process_load,
					     unsigned long, value_p, nr,
...
static BEGIN_GSCHEDULER_PROBE_SOURCE_TYPE(value_process_load),
	.GSCHEDULER_PROBE_SOURCE_GET(value_process_load),
	.GSCHEDULER_PROBE_SOURCE_VALUE_TYPE(value_process_load, unsigned long),
	.GSCHEDULER_PROBE_SOURCE_PARAM_TYPE(value_process_load, pid_t),
END_GSCHEDULER_PROBE_SOURCE_TYPE(value_process_load);
...
static GSCHEDULER_PROBE_TYPE(mosix_probe_type, NULL, NULL /* measure_mosix */);
...
int mosix_probe_init(void)
{
	...
	mosix_probe_sources[VALUE_MEAN_LOAD] =
		gscheduler_probe_source_create(&value_mean_load_type,
					      "mean_load");
        mosix_probe_sources[VALUE_UPPER_LOAD] =
		gscheduler_probe_source_create(&value_upper_load_type,
					      "upper_load");
        mosix_probe_sources[VALUE_SINGLE_PROCESS_LOAD] =
		gscheduler_probe_source_create(&value_single_process_load_type,
					      "single_process_load");
        mosix_probe_sources[VALUE_NORM_MEAN_LOAD] =
		gscheduler_probe_source_create(&value_norm_mean_load_type,
					      "norm_mean_load");
        mosix_probe_sources[VALUE_NORM_UPPER_LOAD] =
		gscheduler_probe_source_create(&value_norm_upper_load_type,
					      "norm_upper_load");
		...
		gscheduler_probe_source_create(&value_process_load_type,
					      "process_load");
	...
	mosix_probe = gscheduler_probe_create(&mosix_probe_type,
					     MOSIX_PROBE_NAME,
					     mosix_probe_sources,
					     NULL);
	if (mosix_probe == NULL) {
...
/*	prev_jiffies = get_jiffies_64(); */
	...
	err = hcc_gsched_module_info_register(&mosix_probe_module_info_type);
	if (err)
		goto err_mod_info;

	err = gscheduler_probe_register(mosix_probe);
	if (err)
		goto err_register;
...
err_hooks:
	gscheduler_probe_free(mosix_probe);
out_kmalloc:
	for (i=0; i<NR_VALUES; i++)
		if (mosix_probe_sources[i])
			gscheduler_probe_source_free(mosix_probe_sources[i]);
	goto out;
...
void mosix_probe_exit(void)
{
	...
	if (!probe_not_registered)
		gscheduler_probe_unregister(mosix_probe);

	if (!mod_info_not_registered)
		hcc_gsched_module_info_unregister(&mosix_probe_module_info_type);
