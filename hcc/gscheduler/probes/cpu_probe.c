/*
 *  hcc/gscheduler/probes/cpu_probe.c
 *
...
static struct gscheduler_probe *cpu_probe;
static int active;
...
DEFINE_GSCHEDULER_PROBE_SOURCE_GET_WITH_INPUT(cpu_probe_load,
...
DEFINE_GSCHEDULER_PROBE_SOURCE_SHOW(cpu_probe_load, page)
{
	size_t size = GSCHEDULER_PROBE_SOURCE_ATTR_SIZE;
	ssize_t count;
	int tmp_count;
	int i;
...
DEFINE_GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(cpu_probe_load)
{
	int i;
	int isChanged = 0;
...
static BEGIN_GSCHEDULER_PROBE_SOURCE_TYPE(cpu_probe_load),
	.GSCHEDULER_PROBE_SOURCE_GET(cpu_probe_load),
	.GSCHEDULER_PROBE_SOURCE_SHOW(cpu_probe_load),
	.GSCHEDULER_PROBE_SOURCE_VALUE_TYPE(cpu_probe_load, unsigned long),
	.GSCHEDULER_PROBE_SOURCE_PARAM_TYPE(cpu_probe_load, int),
	.GSCHEDULER_PROBE_SOURCE_HAS_CHANGED(cpu_probe_load),
END_GSCHEDULER_PROBE_SOURCE_TYPE(cpu_probe_load);

static struct gscheduler_probe_source *cpu_probe_sources[2];

static ssize_t active_attr_show(struct gscheduler_probe *probe, char *page)
{
	return sprintf(page, "%d", active);
}

static ssize_t active_attr_store(struct gscheduler_probe *probe,
...
static GSCHEDULER_PROBE_ATTRIBUTE(active_attr, "active", 0644,
				 active_attr_show, active_attr_store);

static struct gscheduler_probe_attribute *cpu_probe_attrs[] = {
	&active_attr,
	NULL
};

static GSCHEDULER_PROBE_TYPE(cpu_probe_type, cpu_probe_attrs, measure_cpu);
...
int init_module()
{
	...
	if (probe_data == NULL || probe_data_prev==NULL || cpu_used==NULL ||
        ...
        cpu_probe_sources[0] = gscheduler_probe_source_create(
		&cpu_probe_load_type,
		"cpu_usage");
	...
	cpu_probe = gscheduler_probe_create(&cpu_probe_type, CPU_PROBE_NAME,
	...
	err = gscheduler_probe_register(cpu_probe);
	if (err)
		goto err_register;
...
err_register:
	gscheduler_probe_free(cpu_probe);
out_probe_sources_init:
	if (cpu_probe_sources[0] != NULL)
		gscheduler_probe_source_free(cpu_probe_sources[0]);
...
void cleanup_module()
{
	...
	gscheduler_probe_unregister(cpu_probe);
	gscheduler_probe_free(cpu_probe);
	for (i = 0; cpu_probe_sources[i] != NULL; i++)
		gscheduler_probe_source_free(cpu_probe_sources[i]);
	kfree(probe_data);
	kfree(probe_data_prev);
	kfree(cpu_used);
	kfree(cpu_total);
