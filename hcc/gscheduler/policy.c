...
static
inline
struct gscheduler_policy *to_gscheduler_policy(struct config_item *item)
{
	return container_of(to_config_group(item),
			    struct gscheduler_policy, group);
}

static
inline
struct gscheduler_policy_attribute *
to_gscheduler_policy_attr(struct configfs_attribute *attr)
{
	return container_of(attr, struct gscheduler_policy_attribute, attr);
}

static
inline
struct gscheduler_policy_type *
to_gscheduler_policy_type(struct config_item_type *type)
{
	return container_of(type, struct gscheduler_policy_type, item_type);
}
 ...
 * @author Innogrid HCC
 */
static ssize_t gscheduler_policy_attribute_show(struct config_item *item,
					       struct configfs_attribute *attr,
					       char *page)
{
	struct gscheduler_policy_attribute *policy_attr =
		to_gscheduler_policy_attr(attr);
	struct gscheduler_policy *policy = to_gscheduler_policy(item);
	ssize_t ret = -EACCES;
 ...
 * @author Innogrid HCC
 */
static ssize_t gscheduler_policy_attribute_store(struct config_item *item,
						struct configfs_attribute *attr,
...
static void gscheduler_policy_release(struct config_item *);

static struct global_config_attrs *policy_global_attrs(struct config_item *item)
{
	return &to_gscheduler_policy(item)->global_attrs;
}
...
struct global_config_item_operations policy_global_item_ops = {
	.config = {
		.release = gscheduler_policy_release,
		.show_attribute = gscheduler_policy_attribute_show,
		.store_attribute = gscheduler_policy_attribute_store,
	},
			  ...
			  struct config_group *def_groups[])
{
	...
	memset(policy, 0, sizeof(struct gscheduler_policy));
...
/*
 * Configfs callback when the last reference on the gscheduler_policy is dropped.
 * Destroys the scheduling policy.
 */
static void gscheduler_policy_release(struct config_item *item)
{
	struct gscheduler_policy_type *type =
		to_gscheduler_policy_type(item->ci_type);
	struct gscheduler_policy *policy = to_gscheduler_policy(item);
 ...
 */
static
struct gscheduler_policy_type *gscheduler_policy_type_find(const char *name)
{
	struct list_head *pos;
	struct gscheduler_policy_type *entry;

	list_for_each(pos, &policies_list) {
		entry = list_entry(pos, struct gscheduler_policy_type, list);
 ...
 */
static int gscheduler_policy_attribute_array_length(
...
int gscheduler_policy_type_register(struct gscheduler_policy_type *type)
{
	...
	num_attrs = gscheduler_policy_attribute_array_length(type->attrs);
	if (num_attrs) {
	...
	if (gscheduler_policy_type_find(type->name) != NULL) {
		ret = -EEXIST;
	} else {
		/*
	...
	if (ret)
		...
		printk(KERN_INFO
		       "successfully registered gscheduler_policy_type %s\n",
		       type->name);
...
{
	struct gscheduler_policy_type *type;
	struct gscheduler_policy *tmp_policy;
	int err;

	spin_lock(&policies_lock);
	type = gscheduler_policy_type_find(name);
	if (!type) {
		spin_unlock(&policies_lock);
...
EXPORT_SYMBOL(gscheduler_policy_type_register);
EXPORT_SYMBOL(gscheduler_policy_type_unregister);
EXPORT_SYMBOL(gscheduler_policy_init);
EXPORT_SYMBOL(gscheduler_policy_cleanup);
